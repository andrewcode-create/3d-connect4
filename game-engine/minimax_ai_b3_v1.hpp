...
struct connect3dBoardFast : public mm3::board_t<connect3dMoveFast>{
    ...
    */
    ...
    struct MoveFactory {
        ...
        connect3dMoveFast getNextBestMove() {
            ...
            if (!heuristicCalculated && board) {
                ...
                double multiplier = (p == player::A) ? 1.0 : -1.0;
                for (int i = 0; i < count; ++i) {
                    board->makeMove(moves[i]);
                    double diff = board->heuristic() - initH;
                    moves[i].deltaHeuristic = diff * multiplier;
                    board->undoMove(moves[i]);
                    //moves[i].hasHeuristic = true; KEEP COMMENTED OUT, FLOATING POINT ERROR
                }
    ...
    static inline double scoreFromCounts(int cntA, int cntB) {
    ...
    connect3dBoardFast(const connect3dBoard& b) {
        ...
        for (int i = 0; i < 64; ++i) {
            if (b.board[i] == player::A) {
                boardA |= (1ULL << i);
                zHash ^= keys.piecesA[i];
            }
            else if (b.board[i] == player::B) {
                boardB |= (1ULL << i);
                zHash ^= keys.piecesB[i];
            }
        }
        playerTurn = b.playerTurn;
        if (playerTurn == player::B) zHash ^= keys.sideToMove;
    ...
    void makeMove(connect3dMoveFast m) override {
        ...
        if (m.hasHeuristic) {
            ...
            double diff = (playerTurn == player::A) ? m.deltaHeuristic : -m.deltaHeuristic;
            currentScore += diff * 10000.0;
        } else {
        ...
        if (playerTurn == player::A) {
            boardA |= moveMask;
            zHash ^= keys.piecesA[idx];
        } else {
            boardB |= moveMask;
            zHash ^= keys.piecesB[idx];
        }
        zHash ^= keys.sideToMove;
    ...
    void undoMove(connect3dMoveFast m) override {
        ...
        if (m.hasHeuristic) {
            ...
            double diff = (playerTurn == player::B) ? m.deltaHeuristic : -m.deltaHeuristic;
            currentScore -= diff * 10000.0;
        } else {
            int idx = m.movenum + (layer << 4);
            const auto& lines = getLookup().second[idx];
...
class MinimaxAI_b3_v1 : public AI_base {
public:
    std::vector<mm3::TTEntry> tt;

    MinimaxAI_b3_v1() {
        tt.resize(1024 * 1024 * 4); // 4MB
    }

    evalReturn getNextMove(connect3dBoard board) override {
        b3_v1::connect3dBoardFast adapter(board);
        mm3::stat_t stats;
        b3_v1::connect3dMoveFast bestMove;
        
        // Depth 4 provides a good balance of strength and speed for branching factor 16
        int depth = 9; 

        double score = mm3::minimax(adapter, board.getPlayerTurn(), 0, depth, &bestMove, stats, tt);
